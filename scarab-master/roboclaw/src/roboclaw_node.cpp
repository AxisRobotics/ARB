...
public:
  explicit DifferentialDriver(const ros::NodeHandle &node) :
    ...
    nh_.param("wheel_diam", wheel_diam_, 0.127); // 0.085
    nh_.param("quad_pulse_per_motor_rev", quad_pulse_per_motor_rev_, 64.0);
    nh_.param("motor_to_wheel_ratio", motor_to_wheel_ratio_, 30.0); // 100.0
    double motor_rev_per_meter = motor_to_wheel_ratio_ / (M_PI * wheel_diam_); //75.191
    quad_pulse_per_meter_ = quad_pulse_per_motor_rev_ * motor_rev_per_meter; //4812

    // Controller
    nh_.param("max_wheel_vel", max_wheel_vel_, 1.0); //.5
    nh_.param("min_wheel_vel", min_wheel_vel_, 0.00);
    nh_.param("accel_max", accel_max_, 1.0); //.5
    accel_max_quad_ = accel_max_ * quad_pulse_per_meter_;
    nh_.param("pid_p", pid_p_, 0.9); //30.9
    nh_.param("pid_i", pid_i_, 3.0);
    nh_.param("pid_d", pid_d_, 0.1);
    nh_.param("pid_iclamp", pid_iclamp_, 500.0); //2000
    ...
    nh_.param("portname", portname_, std::string("/dev/ttyACM0"));
    ...
    // To calculate: (shaft rpm * gear ratio * encoder ticks per rev)/60s
    // Our motor specs: 102.083:1   6533 CPR   100 RPM @ 12v
    // (100 * 102 * 6533) / 60
    //duty_per_qpps_ = 3200; //(100 * 102 * 64) / 60; //500000
    qpps_ = 250000;
    //duty_per_qpps_ = 512.0 / qpps_;
    duty_per_qpps_ = qpps_;
  }
  ...
  void setVel(double v, double w, bool publish = true) {
    double wmag = fabs(w);
    double vmag = fabs(v);
    ...
    ROS_INFO("duty_sp: %2.2f %2.2f", state_.left_duty_sp, state_.right_duty_sp);
  ...
  void update() {
    ...
    if (abs(state_.left_duty) > MAX_DUTY) {
      state_.left_duty = copysign(MAX_DUTY, state_.left_duty); //mag of x, sign of y
    }
    if (abs(state_.right_duty) > MAX_DUTY) {
      state_.right_duty = copysign(MAX_DUTY, state_.right_duty);
    }

    double dlimit = 100; //orig: 10 effects acceleration
...
public:
  RoboClawNode() : priv_node_("~") {
    ...
    priv_node_.param("freq", freq_, 180.0); //default: 30 
  ...
  void OnTwistCmd(const geometry_msgs::TwistConstPtr &input)  {
    ROS_INFO("Got cmd_vel: %2.2f %2.2f", input->linear.x, input->angular.z);
    {
      boost::mutex::scoped_lock lock(driver_mutex_);
